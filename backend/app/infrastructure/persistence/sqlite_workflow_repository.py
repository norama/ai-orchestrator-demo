...
class SqliteWorkflowRepository(WorkflowRepository):
        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.execute(
                ...
                CREATE TABLE IF NOT EXISTS workflows (
                    ...
                    name TEXT,
                    description TEXT,
                    domain_type TEXT NOT NULL CHECK (domain_type IN ('PARROT', 'PRINTER', 'LLM_SUPPORT')),
    ...
    def persist(self, workflow: WorkflowState) -> WorkflowState:
        with sqlite3.connect(self.db_path) as conn:
            conn.execute(
                """
                INSERT INTO workflows (id, name, description, domain_type, phase, state_json, updated_at)
                VALUES (?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    str(workflow.id),
    ...
    def save(self, workflow: WorkflowState) -> WorkflowState:
        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.execute(
                """
                UPDATE workflows
                SET name = ?, description = ?, domain_type = ?, phase = ?, state_json = ?, updated_at = ?
                WHERE id = ?
                """,
                (
                    workflow.name,
